...
| --- | --- | --- |
| `d3fde9a` | Skills and agents now run on the session model by default: agents moved from `model: sonnet`/`haiku` to `model: inherit`, skills dropped `model: sonnet`, and Codex agents dropped their `gpt-5.6-terra`/`gpt-5.6-luna` pins. Only `adversarial-debate` and `finding-verifier-high` still pin a model (`opus` / `gpt-6-sol`, up from `gpt-5.6-sol`). The Codex session model moved to `gpt-6-sol`. `check-agent-drift` no longer requires `codex-model` on runners or `model` on the `my-workflow` coordinator. | Before this, every skill was capped at Sonnet/Terra regardless of session model. On Codex, the low/high verifier pair now shares `gpt-6-sol` and differs only by reasoning effort (medium vs xhigh); if tier separation matters again, re-add a cheaper `codex-model:` to `finding-verifier-low`. Known-good prior state: `58781ca`. |
