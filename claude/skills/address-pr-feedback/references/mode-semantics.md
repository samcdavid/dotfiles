...
- Gate: **present triage and wait for confirmation** before changing code. Real reviewers are on the other end; pushing back on a colleague's comment is a judgment call the user owns.
- Output: evidence-backed replies drafted per comment. Never published unless explicitly asked.
...
Each validated fix lands as its own local commit via the `commit` skill, scoped to that fix's files. A fix that fails validation stays uncommitted. Pushing, publishing replies, and resolving threads remain gated on an explicit request in both modes.
