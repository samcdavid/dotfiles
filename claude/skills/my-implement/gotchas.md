...
- **Wrong:** Using a guessed or lowercase form of a brand name (e.g. writing `brandname` when the correct form is `BrandName`)
...
- **Source:** Local pytest claimed 378 MCP tests passing on a fastmcp downgrade pin. CI then revealed 378 failures because the pin actually broke decorator semantics. Root cause: `apps/<sub-app>/.venv/bin/pytest` shebanged to a sibling worktree's `apps/<sub-app>/.venv/bin/python` even though pwd was the feature-branch worktree — every test was running against the sibling's stale fastmcp 3.3.1, not the feature branch's freshly-pinned 2.14.7.

### Axon Ecto queries use pipe style
- **Category:** convention
- **Context:** Writing or modifying Ecto queries in Axon.
- **Wrong:** Introducing or retaining equivalent `from(...)` queries because they are idiomatic or a conversion seems behavior-neutral.
- **Right:** Use `Schema |> join(...) |> where(...) |> select(...) |> repo.all()`; reserve `from(...)` for documented construct-specific exceptions.
- **Why:** Pipe composition is the agreed Axon code style and keeps complex query transformations locally readable.
- **Source:** MCP-769 review correction.
