...
- **Category:** failure-mode
- **Context:** Any PR review — `my-review` itself and every subagent it spawns (research subagents codebase-analyzer / codebase-pattern-finder / docs-researcher, the per-lens reviewer agents, and adversarial-debate). **Especially adversarial-debate**, which historically has been the leakage path that creates stale `pr-*` refs.
