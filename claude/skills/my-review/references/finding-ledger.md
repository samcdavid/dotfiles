...
| `review-handoff.local-sensitive-changes` | accepted | Local advisory handoff acknowledgement | User explicitly acknowledged the config/infra/suppression anchors; accepted scope: `<category>:<path>:<changed_content_digest>`, review base `<sha>` | user acknowledgement | 2026-08-27, Local Review Confirmation |
| `review-handoff.operational-readiness` | accepted | Operational readiness confirmation | User explicitly confirmed env vars and flags are set appropriately in every staging/production environment and migrations were tested successfully in staging; confirmed scope: `<category>:<path>:<changed_content_digest>`, review base `<sha>` | user confirmation | 2026-08-28, Local Review Confirmation |
```
...
- `accepted` — only for `change-set-risk.md`'s local human confirmations. For
...
Suppress a local confirmation when every current tuple is covered by its
...
HEAD when available; and `Recorded: <date>, Local Review Confirmation`. For the
