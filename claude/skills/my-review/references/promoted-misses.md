...
- **Proposed promotion** — `target: references/general-checklist.md` (Observability subsection) and `~/.claude/skills/my-validate/references/protocol.md` (Observability Validation Step 2); `wording:` "Trace the actual value behind new telemetry, not just its call site — a hardcoded/rarely-populated field makes an index or metric permanently empty even when the emission code is correct, and a metric type's assumed semantics must be checked against the specific backend, not assumed."
