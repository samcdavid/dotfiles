...
implementation authorization and dispatch `my-implement`, followed by
...
5. Dispatch `my-implement` with the ledger as the approved plan and test
   strategy. Let it complete every phase and holistic test gate.
6. Dispatch `implement-review` only after implementation completion; preserve
   its bounded review/repair loop unchanged.

Update the ledger after every planning turn, gate, implementation phase result,
and review outcome. Local commits are expected from the existing execution
