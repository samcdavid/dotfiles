...
| 8-10 | gated atomic block: `my-implement` -> automatic fix loop | approved plan + completed stage 1-7 ledger | commits per phase, validation report, review verdict | stop after final review |
...
- **9 `my-validate`:** Run in **Plan Mode** against the plan file from stage 4. Let it self-repair trivial failures; escalate what it cannot fix confidently.
- **Compute the diff once for the review stage.** Detect the base branch and the changed files, then pass both to `my-review`:
  ...
  ```
- **10 `my-review`:** The single, consolidated review stage. Invoke it with the base branch name so it diffs the current work tree vs `main`/`master`; stay in read-only local review — no checkout, no PR. Because this is the deliberate full pipeline, don't let lens triage thin the review:
